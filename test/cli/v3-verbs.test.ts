...
/**
 * MERGE-TIME (lanes 02c + 03): `events` is claimed by both CLI entry points.
 *
 * `twining-mcp events` is lane 02c's database-free reader over the retained
 * archive, which is what keeps a ROLLED-BACK store inspectable (ADR §10.7).
 * `twining events` is lane 03's JSON-envelope reader over the live log, and it
 * wins on that entry point. The seam that must not open is a rolled-back store
 * reporting itself as never-migrated from the `twining` CLI.
 */
describe("events on both entry points", () => {
  it("routes `twining events` to the v3 verb, not to lane 02c's subcommand", () => {
    const d = classifyCliArgv(["node", "twining", "events", "ls"]);
    expect(d.kind).toBe("v3");
    expect(d.kind === "v3" && d.name).toBe("events");
  });

  it("names the database-free reader when the store is rolled back, instead of 'never migrated'", async () => {
    // A rolled-back store: the event archive is retained, the v3 descriptor is not.
    fs.mkdirSync(path.join(fx.twiningDir, "events", "2026-09"), { recursive: true });
    fs.writeFileSync(path.join(fx.twiningDir, "events", "2026-09", "01JZZZZZZZZZZZZZZZZZZZZZZZ.json"), "{}\n");
    expect(isV3Store(fx.twiningDir)).toBe(false);

    const out = await runEvents(["ls"], fx.projectRoot, fx.env);
    expect(out.exitCode).toBe(1);
    expect(out.error!.code).toBe("STORE_ROLLED_BACK");
    expect(out.error!.message).toContain("twining-mcp events");
  });

  it("still reports a never-migrated store as STORE_NOT_V3", async () => {
    expect(fs.existsSync(path.join(fx.twiningDir, "events"))).toBe(false);
    const out = await runEvents(["ls"], fx.projectRoot, fx.env);
    expect(out.exitCode).toBe(1);
    expect(out.error!.code).toBe("STORE_NOT_V3");
  });
...
/**
 * MERGE-TIME (lanes 02b + 03): the ceremony's own output must be admissible.
 *
 * `rule` capability is judged against the membership projected from the
 * event's causal ANCESTORS (never the replica's latest policy), so a ruling
 * that does not descend from the policy the ceremony just established is
 * quarantined `no_policy_yet`. This pins the citation rather than the symptom.
 */
describe("the ceremony cites the policy it is judged under", () => {
  it("names the bootstrap membership among the ruling's parents", async () => {
    const env = { ...fx.env, TWINING_HUMAN_PASSPHRASE: "a-good-passphrase" };
    await runIdentity(["init", "--human"], fx.projectRoot, env);
    const human = unlockHumanIdentity(listHumanIdentities(env)[0]!, "a-good-passphrase");
    const runtime = openRuntime({ projectRoot: fx.projectRoot, env });
    try {
      const out = await appendRuling(runtime, human, {
        scope: { ...runtime.scope, path: "src/auth/" },
        statement: "tokens expire in 15 minutes",
      });
      expect("id" in out, JSON.stringify(out)).toBe(true);
      const rulingId = (out as { id: string }).id;
      await runtime.store!.admit();
      await runtime.store!.project();

      const events = await runtime.store!.events({});
      const ruling = events.find((e) => e.id === rulingId)!;
      const membership = events.find((e) => e.record?.type === "membership")!;
      expect(ruling.parents).toContain(membership.id);

      // And therefore it is admitted, not quarantined.
      const state = await runtime.store!.deliveryState(rulingId);
      expect(state!.state).not.toBe("quarantined");
    } finally {
      runtime.close();
    }
  });
