...
| OVERRIDE-03 | SHOULD | Commit `.twining/` promptly after lifecycle writes (override/promote/archive) — the records-tree rewrite is an uncommitted working-tree change until then, and a git operation that discards it makes the next file-wins ingest revert the write (reported as `lifecycle_reverts`) |
