...
- **`twining capabilities`** — every command with its surface flag, description, and a JSON Schema derived from the zod shape, plus `server_version`, `schema_version`, and the resolved `project_root` / `store_dir` / `store_writable`. It never creates a store, so it is safe (and useful) to run in a read-only tree.
...
- **MCP behavior is unchanged**: same tool names, descriptions, input schemas, response JSON, surface gating, and error codes. The nine handlers whose `catch` deliberately had no `TwiningError` branch keep mapping everything to `INTERNAL_ERROR`.
