...
A `twining` command-line interface, so an agent host that cannot whitelist an
MCP server can still use Twining. The CLI is not a second implementation: every
Twining verb moved into a shared, transport-agnostic command core that the MCP
server now registers from, so the two front ends cannot drift.

### Added
- **`twining` CLI** (`npm i -D twining-mcp` → `npx twining <command>`, or install globally). `twining <command> [--json '<json>' | --input-file <f> | --stdin] [--project <dir>] [--agent-id <id>]`. Command names are the `twining_*` tool names with the prefix optional. Output is exactly **one JSON envelope** on stdout (`{"ok":true,"schema_version":"1","server_version":"…","command":"…","result":{…}}`) with every diagnostic on stderr; `result` is byte-identical to the matching MCP tool's payload. Exit codes: `0` success, `1` the command ran and failed, `2` the invocation was wrong (usage, unknown command, unparseable JSON, schema-invalid input). `--version` and `--help` print plain text, since they describe the binary rather than invoking a command.
- **`twining capabilities`** — every command with its surface flag, `requires_mode` where config `tools.mode` must be `"full"` for an MCP peer to have it at all (`twining_status`, `twining_archive`, and the five graph commands), description, and a JSON Schema derived from the zod shape, plus `server_version`, `schema_version`, and the resolved `project_root` / `store_dir` / `store_writable`. It never creates a store, so it is safe (and useful) to run in a read-only tree.
- **`migrate` and `validate-records` are available under `twining` too**, with their own output and exit codes unchanged. `twining-mcp` keeps both subcommands exactly as before.
- **Shared command core** — `src/core/context.ts` (`createTwiningContext`: config, stores, embedder, engines, sync manager, pending processor, without the McpServer, dashboard or telemetry) and `src/core/commands/*` (39 command definitions: name, description, zod input shape, surface, handler). `src/tools/*.ts` are now registration loops over those definitions; `createServer` composes the context and adds MCP registration. Public `registerXTools` signatures are unchanged.

### Behavior
- **Every success envelope names the store it wrote** (`project_root`, `store_dir`, absolute). The linked-worktree redirect — a cwd-default call inside a worktree targets the MAIN checkout's store — was otherwise invisible on the happy path, and writing to the wrong store is the one failure the CLI cannot detect for you. Paths are absolutized for reporting only; resolution is unchanged.
- **An unrecognized flag or a stray positional argument is refused (exit 2), never ignored.** `twining twining_archive --jsonn '{"retain":200}'` used to drop the payload and run an argument-free archive — which sweeps the whole board. A dropped payload must never look like a successful call.
- **The CLI never reaches the network.** When the local ONNX model is absent it goes straight to keyword search and says so on stderr instead of attempting a download — the difference between a fast, honest fallback and a multi-second hang in a sandbox with no network. A cached model is still used, with remote fetches disabled. `Embedder` takes an `offline` option; the MCP server's behavior is unchanged.
- **The CLI refuses to write to a store it cannot write.** If the resolved `.twining` directory (or the project root it would be created in) is not writable, the command exits 1 with `STORE_UNWRITABLE`, naming the path and the two explicit fixes (`--project` / `TWINING_PROJECT`, or `TWINING_WORKTREE_LOCAL=true` for the linked-worktree-with-read-only-main case). It never silently falls back to another store.
- **`.twining/.last-record` is written by the commands, not the MCP layer**, so `twining record` / `post` / `decide` satisfy the pre-commit Gate 2 check from a shell exactly as they do over MCP.
- **The git-staleness probe runs on both fronts.** The CLI calls the sync manager's `maybeResync()` before dispatch, so a command issued after a pull sees the pulled records — the same probe the MCP server patches onto every tool call.
- **The CLI skips the server's background work** (pending-queue drain and its 60s timer, sqlite startup embedding reconcile): a process that exits in milliseconds must not start writes it cannot finish. The drain is at-least-once and idempotent, so the next server start picks it up. No dashboard is started and no telemetry is initialized on any CLI path.
- **The CLI dispatches every command regardless of `tools.full_surface`.** That gate exists to keep an LLM's tool list short; a shell has no tool list. `capabilities` reports each command's `surface` so you can tell why an MCP peer may not see one. `full_surface` still gates provisional minting inside `twining_record` on both fronts — that is a behavior gate, not a surface gate.
- **MCP behavior is unchanged**: same tool names, descriptions, input schemas, response JSON, surface gating, and error codes. The 15 handlers whose `catch` deliberately had no `TwiningError` branch (`twining_why`, `twining_triage`, `twining_status`, the blackboard read trio, the housekeeping trio, and all six coordination tools) keep mapping everything to `INTERNAL_ERROR`.

### Documentation
- **[docs/CLI.md](docs/CLI.md)** — commands, envelope, exit codes, store resolution, offline behavior, Codex sandbox notes, how hooks should call it, and the full list of CLI-vs-MCP differences.
- README gains a **CLI** section.
