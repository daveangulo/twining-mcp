...
| D2 mutation model (mutations as event records; aggregates cease to be committed) | ⚠️ **PARTIAL** (annotated 2026-09-05) — creates are append-only per-ULID files; mutations rewrite the same file in place; v1 aggregates stay tracked after migrate. Disposition: DD-12 in [plans/2026-09-05-cross-host-sync-plan.md](plans/2026-09-05-cross-host-sync-plan.md) | — |
...
> **[2026-09-05 annotation — preserved, not rewritten]** Partially implemented. Creates are append-only per-ULID files as designed. Mutations are NOT event records: status changes, supersede, link_commit, amend, post resolve, handoff acknowledge and entity/relation upserts rewrite the same ULID file in place (`src/storage/sync/record-export.ts:20-21` states this as the contract; `test/record-sync.test.ts:156-168` pins it), and `twining-mcp migrate` leaves the three aggregates tracked (`src/migrate/cli.ts:122-128` prints only `git add`). Consequence: two hosts that mutate the same record between syncs can conflict on that file — overlapping-key or adjacent-line edits do (measured 2026-09-05); disjoint, non-adjacent edits merge textually but are semantically unverified (see UPGRADE-v2 "Merging `.twining`"). Whether to keep this model, implement event records, or add a revision stamp is design decision **DD-12** (`plans/2026-09-05-cross-host-sync-plan.md` §4.7).
...
> **[2026-09-05 annotation — preserved, not rewritten]** Not implemented as written. The daemon was never built (W4), and `RecordExporter` writes under the resolved project root, which for a linked worktree is the MAIN checkout (`src/storage/sync/record-export.ts:63-65`, `src/utils/project-root.ts:86-91`) — a worktree session's records land in the main checkout's working tree, not its own. This is what makes one per-repo drain feasible and is where any records commit must look. Recorded as a deviation; transport boundary questions are **DD-13** in the same plan.
