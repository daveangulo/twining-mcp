...
The Twining plugin includes five hooks that enforce the lifecycle gates. All five share the same guards: they no-op when `TWINING_DISABLED=true` and when the project has no `.twining/` directory, and they **fail open** on a *missing* sentinel or marker, unparseable hook input, or an absent git binary — a coordination tool must never be the reason a commit or session-exit is impossible. One deliberate exception: a `.last-record` sentinel that exists but is unreadable or empty parses as epoch 0, so the gating hooks fail *toward gating* there (every commit/exit blocks until a successful record rewrites it, or `TWINING_DISABLED=true`) — a corrupt sentinel is treated as "you have not recorded", not as permission.
...
Writes epoch-seconds to `.twining/.sessions/<session_id>` after every successful edit **whose target path belongs to this store's project** — the marker the Stop gate reads. Since plugin 1.33.0 the edit-path filter canonicalizes both sides through the nearest existing ancestor (`pwd -P`), so symlinked roots (`/tmp` → `/private/tmp`, symlinked homes) match correctly. Worktrees are session-relative: a session *running in* a linked worktree stamps for edits under its own worktree root, while a main-checkout session's edits into subagent trees under `.claude/worktrees/` deliberately do **not** stamp (a controller is not on the hook for its subagents' files). Out-of-tree writes — scratch notes in `/tmp`, files in other repos — never stamp the marker, so a read-only session that takes notes elsewhere is never Gate-2 blocked. An absent or unparseable `file_path` stamps anyway (fail toward gate integrity).
...
- **Server:** connecting the MCP server to a **fresh** checkout still initializes `.twining/` (directories, `config.yml`, `.gitignore` reconciliation) at boot, and any sqlite boot creates WAL files. Against an already-initialized store, boot ALSO drains any queued `pending-posts.jsonl`/`pending-actions.jsonl` into the **committed** blackboard and may post ingest lifecycle-revert warnings — check both queues are empty (or move them aside) before an audit boot; only then are the mutations limited to gitignored churn (`metrics.jsonl`, `.sessions/`). A true no-write server mode is a tracked design decision (DD-6, read-audit remediation plan).
