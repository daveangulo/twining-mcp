...
export async function bootstrapHumanMembership(
...
): Promise<{ created: boolean; reason?: string; membershipEvent?: string }> {
  ...
  // `version` is the event id of the latest admitted event on that record, so
  // it is a valid causal ancestor to cite whether the policy was just created
  // or has since been amended.
  if (existing.length > 0) return { created: false, reason: "a membership policy already exists", membershipEvent: existing[0]!.version };
  ...
  await runtime.store.admit();
  await runtime.store.project();
  return { created: true, membershipEvent: ev.id };
}
...
export async function appendRuling(
  ...
  const principalAppended = await runtime.appendRaw(principalEvent, "adapter");
  // First ceremony on a policy-less store establishes the policy; later ones
  // find it already there and change nothing.
  const policy = await bootstrapHumanMembership(runtime, human.principalId);
  ...
  /**
   * THE RULING MUST CITE THE POLICY IT IS JUDGED UNDER.
   *
   * `rule` capability is evaluated against the membership projected from the
   * event's causal ANCESTORS, never against the replica's latest policy, so a
   * ruling that does not descend from the membership this ceremony just
   * established is quarantined `no_policy_yet` — correctly, because an event
   * that cites no policy cannot be judged against one. Naming the membership
   * (and the principal record that introduces the signing key) as parents is
   * what makes the ceremony's own output admissible.
   */
  const parents = [policy.membershipEvent, principalAppended?.id].filter((id): id is string => typeof id === "string");
...
export async function runEvents(args: string[], projectRoot: string, env: NodeJS.ProcessEnv): Promise<VerbOutcome> {
  ...
  if (!isV3Store(twiningDir)) {
    // A ROLLED-BACK store keeps its event archive but is no longer a v3 store.
    // ADR §10.7 requires that archive to stay readable; the reader that needs
    // no database is the twining-mcp one (src/migrate/cli.ts), so say so
    // instead of reporting the store as never-migrated.
    if (fs.existsSync(path.join(twiningDir, "events"))) {
      return {
        exitCode: 1,
        error: {
          code: "STORE_ROLLED_BACK",
          message:
            "this store is rolled back to v2; its retained event archive is readable with `twining-mcp events ls` / `twining-mcp events show <id>` (ADR §10.7)",
        },
      };
    }
    return {
