/**
 * The unified recording command: twining_record.
 ...
 * Extracted verbatim from src/tools/record-tools.ts.
 */
...
export interface RecordCtx {
  ...
  projectRoot: string;
  twiningDir: string;
  /** config tools.full_surface — gates provisional minting (not registration). */
  fullSurface?: boolean;
}

// One quality nudge per process (i.e. per stdio server session, or per CLI
// invocation — a shell front end has no longer-lived scope to hang it on).
...
const { define } = commandFactory<RecordCtx>();

export const recordCommands: CommandDef<RecordCtx>[] = [
  define({
    name: "twining_record",
    surface: "default",
    description:
    ...
    async handler(ctx, args) {
      const fullSurface = ctx.fullSurface ?? false;
      const blackboardEngine = ctx.blackboardEngine;
      const decisionEngine = ctx.decisionEngine;
      ...
      const scope = args.scope ?? inferScopeFromGit(ctx.projectRoot) ?? "project";
