/**
 * Agent-coordination commands: twining_agents, twining_register,
 * twining_discover, twining_delegate, twining_handoff, twining_acknowledge.
 * Extracted verbatim from src/tools/coordination-tools.ts.
 */
import { z } from "zod";
...
const { define } = commandFactory<CoordinationCtx>();

export const coordinationCommands: CommandDef<CoordinationCtx>[] = [
  define({
    name: "twining_agents",
    surface: "full",
    ...
    async handler(ctx, args) {
      ...
      const agents = await ctx.agentStore.getAll();
      const thresholds =
  ...
  define({
    ...
    async handler(ctx, args) {
      const record = await ctx.agentStore.upsert({
      ...
      if (ctx.graphPopulator) {
        await ctx.graphPopulator.onRegister(args.agent_id, args.capabilities, args.role);
      }
      return {
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.coordinationEngine.discover({
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.coordinationEngine.postDelegation({
  ...
  define({
    ...
    async handler(ctx, args) {
      const record = await ctx.coordinationEngine.createHandoff({
      ...
      if (ctx.graphPopulator) {
        await ctx.graphPopulator.onHandoff({
          source_agent: args.source_agent,
          target_agent: args.target_agent,
          scope: args.scope,
          results: args.results,
  ...
  define({
    ...
    async handler(ctx, args) {
      const record = await ctx.coordinationEngine.acknowledgeHandoff(
        args.handoff_id,
        args.agent_id,
      );
      return {
