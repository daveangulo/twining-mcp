/**
 * Decision commands: twining_decide, twining_why, twining_trace,
 * twining_reconsider, twining_override, twining_promote, twining_commits,
 * twining_search_decisions, twining_amend, twining_link_commit.
 * Extracted verbatim from src/tools/decision-tools.ts.
 */
...
const { define } = commandFactory<DecisionCtx>();

export const decisionCommands: CommandDef<DecisionCtx>[] = [
  define({
    name: "twining_decide",
    surface: "full",
    description:
    ...
    async handler(ctx, args) {
      const result = await ctx.decisionEngine.decide(args);
  ...
  define({
    ...
    async handler(ctx, args) {
      ...
      const result = await ctx.decisionEngine.why(args.scope ?? "", {
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.trace(args.decision_id, args.direction);
    },
  }),

  define({
    name: "twining_reconsider",
    surface: "full",
    description:
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.reconsider(
        args.decision_id,
        args.new_context,
        args.agent_id,
      );
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.override(
        args.decision_id,
        args.reason,
        args.new_decision,
        args.overridden_by,
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.promote(
        args.decision_ids,
        args.promoted_by,
      );
    },
  ...
  define({
    ...
    async handler(ctx, args) {
      ...
      const result = await ctx.decisionEngine.getByCommitHash(hash);
      ...
      try {
        execFileSync(
          ...
          {
            cwd: path.dirname(ctx.twiningDir),
            timeout: 3000,
      ...
      return { ...result, commit_exists, message };
    },
  }),

  define({
    name: "twining_search_decisions",
    surface: "full",
    description:
    ...
    async handler(ctx, args) {
      ...
      return await ctx.decisionEngine.searchDecisions(
        args.query,
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.amend({
        id: args.decision_id,
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.decisionEngine.linkCommit(
        args.decision_id,
        args.commit_hash,
        args.agent_id,
      );
