/**
 * Lifecycle commands: twining_status, twining_archive.
 * Extracted verbatim from src/tools/lifecycle-tools.ts.
 */
...
/** Build/backend identity threaded from createTwiningContext so a session can
 * learn which server build and storage backend serve it from the response
 * itself (2026-08-15 field audit: two builds served concurrent sessions on one
...
const { define, defineNoInput } = commandFactory<LifecycleCtx>();

export const lifecycleCommands: CommandDef<LifecycleCtx>[] = [
  defineNoInput({
    name: "twining_status",
    surface: "default",
    ...
    async handler(ctx) {
      const identity: ServerIdentity = ctx.identity ?? {};
      const agentStore = ctx.agentStore ?? null;

      // Get project name from parent directory
      const projectRoot = path.dirname(ctx.twiningDir);
      const project = path.basename(projectRoot);

      // Count blackboard entries
      const { total_count: blackboard_entries } =
        await ctx.blackboardStore.read();
      ...
      // Graph counts
      const entities = await ctx.graphStore.getEntities();
      const relations = await ctx.graphStore.getRelations();
      const graph_entities = entities.length;
      const graph_relations = relations.length;

      // Find last activity timestamp
      const recentEntries = await ctx.blackboardStore.recent(1);
      ...
      const archiveThreshold =
        ctx.config.archive.max_blackboard_entries_before_archive;
      const { entries: allBoardEntries } = await ctx.blackboardStore.read();
      const archivableCount = partitionArchivable(
        allBoardEntries,
        computeResolvedIds(allBoardEntries),
        {
          before: NO_AGE_CUTOFF,
          retain: ctx.config.archive.retain_recent ?? 0,
        },
      ).to_archive.length;
      ...
      if (identity.backend !== "sqlite") {
        const ds = ctx.decisionStore as Partial<{
          repairIndexDesync: (
            execute: boolean,
      ...
      if (agentStore) {
        ...
        const thresholds =
          ctx.config.agents?.liveness ?? DEFAULT_LIVENESS_THRESHOLDS;
        const now = new Date();
  ...
  define({
    ...
    async handler(ctx, args) {
      return await ctx.archiver.archive(args);
    },
  }),
];
