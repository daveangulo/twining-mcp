/**
 * Housekeeping commands: twining_housekeeping, twining_archive_stale,
 * twining_unarchive. Extracted verbatim from src/tools/housekeeping-tools.ts.
 * All three are dry-run / preview by default.
 */
import { z } from "zod";
...
export interface HousekeepingCtx {
...
const { define } = commandFactory<HousekeepingCtx>();

export const housekeepingCommands: CommandDef<HousekeepingCtx>[] = [
  define({
    name: "twining_housekeeping",
    surface: "default",
    ...
    async handler(ctx, args) {
      const result = await ctx.housekeepingEngine.run({
      ...
      if (args.execute && ac && ac.total_junk > 0) {
        ...
        await ctx.blackboardEngine.post({
          entry_type: "finding",
  ...
  define({
    ...
    async handler(ctx, args) {
      ...
      const decisionIndex = await ctx.decisionStore.getIndex();
      ...
      for (const id of args.ids) {
        if (decisionIds.has(id)) {
          try {
            ...
            const write = await ctx.decisionStore.updateStatus(
              id,
              "archived",
      ...
      if (blackboardIds.length > 0) {
        ...
        const dismissed = await ctx.blackboardEngine.dismiss(blackboardIds);
  ...
  define({
    ...
    async handler(ctx, args) {
      ...
      for (const id of args.ids) {
        const decision = await ctx.decisionStore.get(id);
        if (!decision) {
          notFound.push(id);
