...
function toggleTheme() {
  ...
  // Refresh cytoscape styles if the graph explorer is mounted (js/graph-view.js)
  if (typeof window.__twiningGraphThemeChanged === 'function') {
    window.__twiningGraphThemeChanged();
  }
}
...
function toggleView(tab, viewName) {
  ...
  if (tab === 'decisions') {
    ...
    if (viewName === 'timeline' && typeof window.__twiningTimelineShown === 'function') window.__twiningTimelineShown();
  }
  if (tab === 'graph') {
    ...
    if (viewName === 'visual' && typeof window.__twiningGraphShown === 'function') window.__twiningGraphShown();
  }
  if (tab === 'agents') {
...
/* Replaced by the canvas density timeline in js/density-timeline.js
   (Plan Task 11). vis-timeline and all its glue are gone. */
...
/* Graph visual view replaced by js/graph-view.js (Plan Tasks 12-13). */
